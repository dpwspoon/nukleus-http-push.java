...
package org.reaktivity.nukleus.http.push.internal;
...
import org.reaktivity.nukleus.http.push.internal.layouts.ControlLayout;
...
public class Context implements Closeable
{
        ...
        Configuration config)
    {
        try
        {
            ...
            streamsPath(configDirectory.resolve("http-push/streams"));

            sourceStreamsPath(source -> configDirectory.resolve(format("http-push/streams/%s", source)));

            targetStreamsPath((source, target) ->
                configDirectory.resolve(format("%s/streams/http-push#%s", target, source)));

            this.controlRO = controlRW.controlPath(config.directory().resolve("http-push/control"))
